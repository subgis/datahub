 ...
 */
export const DefineRecipeStep = ({ state, updateState, goTo, prev, usecaseTemplates }: StepProps) => {
    const existingRecipeJson = state.config?.recipe;
    const existingRecipeYaml = existingRecipeJson && jsonToYaml(existingRecipeJson);
    const { type } = state;
    const sourceConfigs = getSourceConfigs(usecaseTemplates, type as string);
    const placeholderRecipe = getPlaceholderRecipe(usecaseTemplates, type);
    ...
    const onClickNext = () => {
        ...
        goTo(UsecaseBuilderSteps.DEFINE_DATA);
    };
