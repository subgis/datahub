...
import { UsecaseBuilderState, UsecaseConfig } from './types';
...
function RecipeBuilder(props: Props) {
    const { state, isEditing, displayRecipe, usecaseConfigs, setStagedRecipe, onClickNext, goToPrevious } = props;
    const { type } = state;
    ...
    return (
        <div>
            {!hideDocsHint && isViewingForm && usecaseConfigs ? (
                <IngestionDocumentationHint onHide={() => setHideDocsHint(true)} sourceConfigs={usecaseConfigs} />
            ) : null}
            {(type === LOOKER || type === LOOK_ML) && <LookerWarning type={type} />}
            {type === CSV && <CSVInfo />}
            <HeaderContainer>
                <Title style={{ marginBottom: 0 }} level={5}>
                    {usecaseConfigs?.displayName} Details
                </Title>
                <ButtonsWrapper>
                    <StyledButton
                        type="text"
